...
$('vehicleForm').addEventListener('submit', async (e) => {
  ...
  const { data: existingWarranties } = await supabase.from('warranties').select('id').eq('vehicle_id', vehicleId);
  ...
  if (toDelete.length) {
    await supabase.from('warranties').delete().in('id', toDelete);
  }
  ...
  for (const w of warrantiesDraft) {
    ...
    if (w.id) {
      await supabase.from('warranties').update(wPayload).eq('id', w.id);
    } else {
